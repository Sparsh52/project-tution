...
class Calendar(HTMLCalendar):
	...
	def formatday(self, day, events,user,user_type):
		if user_type=='student':
			...
			d = ''.join(f'<li style="background-color: {random.choice(color_list)}; padding: 4px; margin: 4px; text-align: center; width: {len(event[1]) * 20}px;">{event[0]} <span style="font-weight: bold; font-style: italic;">[{event[2]}]</span></li>' for event in li if event[3] is None)
		else:
			...
			d = "".join(
				f'<li style="background-color: 	#7CFC00; padding: 4px; margin: 4px; text-align: center; width: {len(event[1])*20}px;">{event[0]} <span style="font-weight: bold; font-style: italic; color: white;">[{event[2]}]</span></li>'
				if event[3] is None
				else f'<li style="background-color: #F08080; padding: 4px; margin: 4px; text-align: center; width: {len(event[1]) * 20}px;">{event[0]} <span style="font-weight: bold; font-style: italic; color: white;">[{event[2]}]</span></li>'
				for event in li
			)
		if day != 0:
